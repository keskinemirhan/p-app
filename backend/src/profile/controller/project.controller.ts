...
export class ProjectController {
  ...
  async getProjectsByProfileId(@Body() reqGetallAttr: ReqGetallAttr) {
    ...
    const projects = await this.projectService.getAll(page, take, {
  ...
  @Post()
  async addProject(
    @Body() reqAddProject: ReqAddProject,
  ...
  async deleteProject(
    ...
    if (!control)
      throw new NotFoundException(generateException("PROJECT_NOT_FOUND"));
    const profile = await this.profileService.getOne({
      account: { id: account.id },
    });
